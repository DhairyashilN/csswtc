...
class NonSujalAmcController extends CI_Controller {
	...
	public function index() {
		if ($this->session->userdata('login')!=1){
			...
			$this->load->view('non_sujal_amc_list',$page_data);
		}
	}
	...
	public function create($id='') {
		if ($this->session->userdata('login')!=1){
			...
			if (isset($id) && !empty($id)) {
				...
				$this->db->where('non_sujal_amc_id', $id);
				$this->db->where('deleted', 0);
				$page_data['ArrAmc']= $this->db->get()->result_array();
			}
			$page_data['active_menu'] = 'samc';
			$this->load->view('non_sujal_amc_details',$page_data);
		}
	}
	...
	public function store($id='') {
		if ($this->session->userdata('login')!=1){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$page_data['non_sujal_amc_id'] = $this->input->post('amc_id');
				...
				if (isset($id) && !empty($id)) {
					$this->db->where('id', $id);
					$this->db->where('non_sujal_amc_id', $this->input->post('amc_id'));
					$this->db->update('non_sujal_amc_items', $page_data);
